	...
	eslint = {
		on_attach = function(client, bufnr)
			...
			vim.api.nvim_create_autocmd("BufWritePre", {
				group = augroup_name,
				buffer = bufnr,
				desc = "Run ESLint fixAll on save for this buffer (ESLint LSP)",
				callback = function()
					-- Use the LSP code action directly; the :EslintFixAll user command
					-- isn't registered on all Neovim/lspconfig versions (would E492).
...
-- Only enable a server whose executable is actually on PATH, so uninstalled
-- servers don't spam "language server is not installed" on every buffer.
-- Explicit exe map (reliable even before nvim-lspconfig's defaults resolve).
local server_exe = {
...
for server, cfg in pairs(servers) do
	if server ~= "jdtls" then
		...
		local exe = server_exe[server] or (type(cmd) == "table" and cmd[1] or nil)
...
------------------------------------------------------------
-- Java/JDTLS setup is wrapped so a missing Java toolchain never breaks LSP startup.
local function _setup_jdtls()
...
-- JDTLS ships per-OS launcher config directories. Pick the right one.
local jdtls_config_name = "config_linux"
if vim.fn.has("win32") == 1 or vim.fn.has("win64") == 1 then
	jdtls_config_name = "config_win"
elseif vim.fn.has("mac") == 1 then
	jdtls_config_name = "config_mac"
end
local config_dir = jdtls_root .. "/" .. jdtls_config_name
